"""Quantro Heart Model result analysis utilities (v1.4.0).
...
class AnalysisParameters:
    ...
    x_axis: str = "lambda"
    output: Path = field(default_factory=lambda: Path("artifacts/rolling_mean.svg"))
    # Use ``auto`` to derive the format from the suffix; only ``svg`` artifacts are permitted.
...
def _resolve_output_format(output_path: Path, requested_format: str) -> str:
    """Determine the export format for the plot based on user intent.

    Parameters
    ----------
    output_path:
        Candidate destination for the figure. The suffix is inspected when
        ``requested_format`` is ``"auto"``.
    requested_format:
        Explicit format requested by the caller. Supported values are
        ``"auto"`` and ``"svg"`` to maintain text-only compliance.

    Returns
    -------
    str
        Resolved matplotlib format identifier.

    Raises
    ------
    ValueError
        If the explicit format is unsupported or the suffix is incompatible.
    """

    normalized_request = requested_format.lower()
    allowed_formats = {"svg"}
    ...
    if normalized_request == "auto":
        ...
        if suffix:
            raise ValueError(
                "Output suffix '.{suffix}' is unsupported; only SVG artifacts are "
                "allowed to satisfy text-only repository policies.".format(
                    suffix=suffix
                )
            )
        LOGGER.debug(
            "No suffix detected for %s; defaulting to SVG export for compliance.",
            output_path,
        )
        return "svg"

    if normalized_request not in allowed_formats:
        raise ValueError(
            "Output format must be either 'svg' or 'auto'; received: "
    ...
    if suffix and suffix != "svg":
        raise ValueError(
            "Output path suffix '.{suffix}' is incompatible with SVG-only exports.".format(
                suffix=suffix
            )
        )
...
def _prepare_output_path(output_path: Path, export_format: str) -> Path:
    """Ensure the output path carries a suffix consistent with ``export_format``."""

    desired_suffix = f".{export_format}"
    current_suffix = output_path.suffix.lower()
    if current_suffix:
        if current_suffix == desired_suffix:
            return output_path
        LOGGER.debug(
            "Replacing suffix %s with %s for output path %s",
            current_suffix,
            desired_suffix,
            output_path,
        )
    ...
    LOGGER.debug(
        "Appending suffix %s to output path lacking an extension: %s",
        desired_suffix,
        output_path,
    )
    return output_path.with_suffix(desired_suffix)
...
def run_analysis(parameters: AnalysisParameters) -> Path:
    ...
    export_format = _resolve_output_format(parameters.output, parameters.output_format)
    final_output = _prepare_output_path(parameters.output, export_format)
...
def plot_results(
    ...
    *,
    ...
    output_path:
        Destination file for the saved plot (suffix must resolve to ``.svg``).
    title:
    ...
    output_format:
        Matplotlib export format identifier (``"svg"`` for text-safe vector output).
    """
    ...
    plt.close(fig)
    LOGGER.info(
        "Saved plot to %s using %s format", output_path, output_format.upper()
    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/rolling_mean.svg"),
        help=(
            "Output path for the generated plot (default: artifacts/rolling_mean.svg). "
            "Only SVG suffixes are supported; --output-format=auto enforces this rule."
        ),
    )
    parser.add_argument(
        "--output-format",
        ...
        choices=["auto", "svg"],
        help=(
            "Image format override. Only SVG outputs are permitted; 'auto' infers from "
            "the suffix or falls back to SVG to satisfy text-only repository policies."
        ),
    )
    parser.add_argument(
        "--verbose",
...
def main() -> None:
    ...
    output_path = run_analysis(parameters)
